...
当任务可以拆成边界明确、能够独立完成的子任务时，使用 sol-luna-workflow skill。Sol 留在主线程，负责理解目标、拆分任务、检查结果和整合输出。优先将代码审查、模块分析、独立实现、测试排查等边界明确的子任务交给使用 Luna Max 的 luna_worker。这是已经选定的默认 worker 路由，不要为每次合格委派重新比较模型档位或请求确认。每个 worker 只接收独立、必要的上下文，不得改变整体目标或自行扩大范围。
